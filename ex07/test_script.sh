...
cc -Wall -Wextra -Werror -o ft_rev_int_tab ft_rev_int_tab.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_rev_int_tab -1 -2 -3 -4 -5"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_rev_int_tab $input"

	output=$(./ft_rev_int_tab $input)
...
run_test "1 2 3 4 5" "5 4 3 2 1" || all_tests_passed=false
run_test "10 20 30" "30 20 10" || all_tests_passed=false
run_test "42" "42" || all_tests_passed=false
run_test "-1 -2 -3 -4 -5" "-5 -4 -3 -2 -1" || all_tests_passed=false
run_test "2 4 6 8" "8 6 4 2" || all_tests_passed=false
