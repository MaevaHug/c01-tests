...
cc -Wall -Wextra -Werror -o ft_swap ft_swap.c main.c
if [ $? -ne 0 ]; then
...
longest_test="$> ./ft_swap 42 24 | cat -e"
...
run_test() {
	...
	echo "$separator"
	echo "$> ./ft_swap $input1 $input2 | cat -e"

	output=$(./ft_swap $input1 $input2 | cat -e)
...
run_test "1" "2" "a = 2, b = 1$" || all_tests_passed=false
run_test "42" "24" "a = 24, b = 42$" || all_tests_passed=false
run_test "-1" "1" "a = 1, b = -1$" || all_tests_passed=false
