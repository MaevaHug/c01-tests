...
cc -Wall -Wextra -Werror -o ft_div_mod ft_div_mod.c main.c
if [ $? -ne 0 ]; then
...
run_test() {
	...
	output=$(./ft_div_mod $a $b)

	div=$(echo "$output" | grep -oP '(?<= = )\d+' | head -1)
	mod=$(echo "$output" | grep -oP '(?<= = )\d+' | tail -1)
...
run_test 10 0 0 0 || all_tests_passed=false  # Test with zero divisor
